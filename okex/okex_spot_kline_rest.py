...
quotes = ['BTC', 'ETH', 'LTC', 'ETC', 'XRP', 'EOS', 'BCH', 'BSV', 'TRX']
...
if __name__ == "__main__":
    okex_spot = OKEX_SPOT_MD(start_time=datetime.now(), end_time=datetime.now())
    start = datetime(2020, 9, 18)
    while start > datetime(2019, 1, 1):
        print(start)
        end = min(start - timedelta(seconds=300), datetime(2019, 1, 1))
        okex_spot.start_datetime = start.timestamp()
        okex_spot.end_datetime = end.timestamp()
