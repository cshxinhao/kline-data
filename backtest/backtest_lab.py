...
import numpy as np
from db_engine import fetch_klines
import time
...
def generate_bar_from_row(row, symbol, exchange):
    ...
    return BarData(
        ...
        open_interest=0,
        volume=row["volume"],
        datetime=datetime.strptime(row['start_datetime'], "%Y-%m-%d %H:%M:%S"),
        gateway_name="DB"
    )
...
class BacktestingEngine:
    ...
    def load_data_csv(self, filename: str, type: str):
        ...
        while start < self.end:
            ...
            if type == 'csv':
                ...
                # a = self.start.timedelta()
            else:
                df = fetch_klines(exchange + '_' + symbol, filename, start, end)
                # df = pd.read_csv(filename)
                df['start_datetime'] = df['start_datetime'].apply(
                    lambda x: time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(x)))
            start_index = self.start.timestamp() - datetime.strptime(df["start_datetime"][0],
                                                                     "%Y-%m-%d %H:%M:%S").timestamp()
            if df is not None and not df.empty:
                for ix, row in df.iloc[int(start_index // 60):].iterrows():
                    if datetime.strptime(row["start_datetime"],
                                         "%Y-%m-%d %H:%M:%S") < start:
                        continue
                    elif datetime.strptime(
                            row["start_datetime"], "%Y-%m-%d %H:%M:%S") > end:
                        break
                    else:
    ...
    def calculate_result(self):
        ...
        self.daily_df.to_csv('daily_profit.csv')
    ...
    def cross_limit_order(self):
        ...
        long_best_price = self.bar.open_price  # open or close
...
engine.load_data_csv('SWAP-BTC/USDT', 'db')
